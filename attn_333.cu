...
void attnWSKCXForNSegmentedKernel(
    ...
    if (wg_idx == 2) {  // producer
        ...
        //smem_i++;
        //gemm_qk(std::integral_constant<size_t, 0>{}, smem_i, phase);
        //gemm_pv(0, true, true, smem_i-1, phase);
        //softmax(std::integral_constant<size_t, 0>{});
        ...
        //for (size_t iw=BN; iw<S; iw+=BN) {
            ...
            gemm_qk(std::integral_constant<size_t, 0>{}, smem_i, phase);
            arrive(&Kempty[smem_i]);
            //if ((iw / BN) % 2 == 1) {arrive(&Kempty[smem_i]);}
            ...
            // const int flag = (((iw / BN) & 1) ^ 1);  // flag=1 表示偶数轮，0 表示奇数轮
            // int prev_smem_i = smem_i ^ flag;
            // int prev_phase  = phase ^ (flag & (smem_i ^ 1));
            ...
            //if ((iw / BN) % 2 == 0) {arrive(&Vempty[prev_smem_i]);}
