...
template <int BM, int BN, int DIM>
struct SMemWS {
    alignas(128) fp16 Q[BM*DIM];
    alignas(128) fp16 K[BN*DIM];
    alignas(128) fp16 V[BN*DIM];
    alignas(128) fp16 O[BM*DIM];
    alignas(8) uint64_t Qmbar;
    alignas(8) uint64_t Kempty;
    alignas(8) uint64_t Vempty;
    alignas(8) uint64_t Kfull;
    alignas(8) uint64_t Vfull;
    
};

template<int BM, int BN, int DIM, int NUM_THREADS>
...
void attnWSKCXForNSegmentedKernel(
    ...
    SMemWS<BM, BN, DIM> &s = *reinterpret_cast<SMemWS<BM, BN, DIM>*>(smem);
    fp16 *sQ = s.Q, *sK = s.K, *sV = s.V, *sO = s.O;
    Barrier *Qmbar = &s.Qmbar,  *Kempty = &s.Kempty, *Vempty = &s.Vempty, *Kfull = &s.Kfull, *Vfull = &s.Vfull;

    // init mbarrier
    if (threadIdx.x == 0) {
        init_barrier(Qmbar, 1);
        init_barrier(Kfull, 1);
        init_barrier(Vfull, 1);
        init_barrier(Kempty, 256);
        init_barrier(Vempty, 256);
    }
    __syncthreads();
    ...
    if (wg_idx == 2) {  // producer
        ...
        if (tid == 256) {
            ...
            for (size_t iw=0; iw<S; iw+=BN, phase ^= 1) {

                // load K
                wait(Kempty, phase);
                expect_bytes(Kfull, BN * DIM * sizeof(fp16));
                load_async(sK, &tensorMapK, Kfull, bs, hn, iw, 0);

                // load V
                wait(Vempty, phase);
                expect_bytes(Vfull, BN * DIM * sizeof(fp16));
                load_async(sV, &tensorMapV, Vfull, bs, hn, iw, 0);
            }
        }
    } else {  // consumer
        ...
        wait(Qmbar, 0);
        static_assert(N == 2, "segmented pipeline expects BM/(2*MMA_M) == 2.");

        fp32 acc_s[BN/QK_MMA_N][QK_MMA_N/16][8];
        fp16 acc_s_cast[BN/QK_MMA_N][QK_MMA_N/16][8];

        auto gemm_qk = [&](auto n_const, int phase) {
            ...
            warpgroup_wait();
            if (n == N-1) { arrive(Kempty); }
        };

        auto gemm_pv = [&](size_t acc_o_i, bool wait_vfull, bool arrive_vempty, int phase) {
            if (wait_vfull) { wait(Vfull, phase); }
            warpgroup_arrive();
            #pragma unroll
            for (size_t j=0; j<DIM/PV_MMA_N; j++) {
                #pragma unroll
                for (size_t k=0; k<BN; k+=MMA_K) {
                    ...
                    fp16 *_VAddr = sV + tma_smem_offset_2d<BN>(v_row, v_col);
            ...
            warpgroup_commit_batch();
            warpgroup_wait();
            if (arrive_vempty) { arrive(Vempty); }
        };
        ...
        // Prologue
        gemm_qk(std::integral_constant<size_t, 0>{}, phase);
        softmax(std::integral_constant<size_t, 0>{});
        gemm_qk(std::integral_constant<size_t, 1>{}, phase);
        gemm_pv(0, true, false, phase);
        softmax(std::integral_constant<size_t, 1>{});

        phase ^= 1;
        // main loop
        for (size_t iw=BN; iw<S; iw+=BN, phase ^= 1) {
            gemm_qk(std::integral_constant<size_t, 0>{}, phase);
            gemm_pv(1, false, true, phase);
            softmax(std::integral_constant<size_t, 0>{});
...
template<int B, int H, int S, int D=128, int BM=128, int BN=128, int NUM_THREADS=384>
void runAttnWSCXForNKernel(fp16 *Q, fp16 *K, fp16 *V, fp16 *O) {
    ...
    auto* kernel = attnWSKCXForNSegmentedKernel<BM, BN, D, NUM_THREADS>;
    constexpr size_t sMemSize = sizeof(SMemWS<BM, BN, D>);
...
// nvcc -std=c++17 -arch=sm_90a -O3 attn_loop_outer_2stage_pipeline.cu -o attn_test -lcuda -Xptxas=-v
