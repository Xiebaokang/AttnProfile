...
void attnWSKCXForNSegmentedKernel(
    ...
    if (wg_idx == 2) {  // producer
        ...
        if (tid == 256) {
            ...
            //for (size_t iw=0; iw<8*BN; iw+=BN, smem_i++) {
        ...
        // registers define
        fp32 acc_o[1][N*DIM/PV_MMA_N][PV_MMA_N/16][8];
        ...
        int counter2 = -1;
        int counter = 0;   // 初始状态：00
        // if (tid == 64 && blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0)
        // for (int i = 0; i < 8; i++) {

        //     counter = (counter + 1) & 0x3;
        //     // 取出两位
        //     int a = (counter >> 1) & 1;   // 高位
        //     int b = counter & 1;          // 低位

        //     printf("counter = %d ab= %d%d\n", counter, a, b);
        ...
        for (size_t iw=1; iw<2*S/BN; iw++) {
        //for (size_t iw=1; iw<16; iw++) {
            ...
            {
                counter2 = (counter2 + 1) & 0x3;
            }

            phase = (counter >> 1) & 1;
            smem_i = counter & 1;
            
            int prev_smem_i = counter2 & 1;
            int prev_phase = (counter2 >> 1) & 1;

            // if (tid == 64 && blockIdx.x == 0 && blockIdx.y == 0 && blockIdx.z == 0){
            // printf("counter = %d, phase =%d, smem_i = %d\n",counter, phase, smem_i);
            // printf("counter2 = %d, phase =%d, smem_i = %d\n",counter2, prev_phase, prev_smem_i);
            // }
            // if ( kk == 0 ) 
            //     { 
            //         counter = (counter + 1) & 0x3;
            ...
            gemm_pv(0, true, true, prev_smem_i, prev_phase, kk1);
...
// nvcc -std=c++17 -arch=sm_90a -O3 attn_222.cu -o attn_test -lcuda -Xptxas=-v
