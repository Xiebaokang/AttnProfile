...
void attnWSKernel(
    ...
    constexpr int NUM_CONSUMER = (NUM_THREADS - 128) / 128;
    constexpr int repeat = BM / MMA_M / NUM_CONSUMER;
    constexpr int NUM_CONSUMER_THREADS = NUM_CONSUMER * 128;
    ...
    if (threadIdx.x == 0) {
        ...
        for (int i = 0; i < NUM_SMEM; ++i) {
            ...
            init_barrier(&Kempty[i], NUM_CONSUMER_THREADS);  // 256 thread arrive
            init_barrier(&Vempty[i], NUM_CONSUMER_THREADS);
        }
    }
    __syncthreads();
    ...
    // TMA load
    if (wg_idx == NUM_CONSUMER) {  // producer
        ...
        // registers define
        fp32 acc_o[repeat][DIM/PV_MMA_N][PV_MMA_N/16][8];
        fp32 scores_max[repeat][2];
        fp32 scores_max_prev[repeat][2];
        fp32 logsum[repeat][2];

        fp32 acc_s[repeat][BN/QK_MMA_N][QK_MMA_N/16][8];
        fp16 acc_s_cast[repeat][BN/QK_MMA_N][QK_MMA_N/16][8];
            ...
            for (size_t i=0; i<repeat; i++) {
                ...
                for (size_t j=0; j<BN/QK_MMA_N; j++) {
                    ...
                    for (size_t k=0; k<DIM; k+=MMA_K) {
                        int q_row = wg_idx * repeat * MMA_M + i * MMA_M;
                        int q_col = k;
        ...
        fence_view_async_shared();
        bar_sync(NUM_CONSUMER_THREADS, 2);
        // tma store
        if (tid == 0) {
...
void attnWSKCXForNKernel(
    ...
    constexpr int NUM_CONSUMER = (NUM_THREADS - 128) / 128;
    static_assert((BM / MMA_M) >= NUM_CONSUMER);
    static_assert((BM / MMA_M) % NUM_CONSUMER == 0);
    assert(S % BN == 0);
    constexpr int NUM_CONSUMER_THREADS = NUM_CONSUMER * 128;
    constexpr int N = BM / NUM_CONSUMER / MMA_M;
    ...
    if (threadIdx.x == 0) {
        ...
        for (int i = 0; i < NUM_SMEM; ++i) {
            init_barrier(&Kfull[i], 1);  // 1 thread arrive
            init_barrier(&Vfull[i], 1);
            init_barrier(&Kempty[i], NUM_CONSUMER_THREADS);
            init_barrier(&Vempty[i], NUM_CONSUMER_THREADS);
        }
    }
    __syncthreads();
    ...
    if (wg_idx == NUM_CONSUMER) {  // producer
        ...
        // Wait all consumer threads before issuing TMA store from tid==0.
        fence_view_async_shared();
        bar_sync(NUM_CONSUMER_THREADS, 2);
        // tma store
        if (tid == 0) {
...
template<int BM, int BN, int DIM, int NUM_THREADS, int NUM_SMEM>
__global__  __launch_bounds__(NUM_THREADS)
void attnWSKCXForN2StageKernel(
    int B, int H, int S, 
    const __grid_constant__ CUtensorMap tensorMapQ, 
    const __grid_constant__ CUtensorMap tensorMapK, 
    const __grid_constant__ CUtensorMap tensorMapV, 
    ...
    constexpr int NUM_CONSUMER_THREADS = NUM_CONSUMER * 128;
    constexpr int N = BM / NUM_CONSUMER / MMA_M;
    static_assert(N == 2, "attnWSKCXForN2StageKernel currently has a hand-scheduled n=0/n=1 pipeline.");
    ...
    if (wg_idx == NUM_CONSUMER) {  // producer
        ...
        auto gemm_pv = [&](size_t acc_o_i, bool wait_vfull, bool arrive_vempty, int smem_i, int phase) {
            fp16 *VAddr = sV + smem_i * BN * DIM;
        ...
        gemm_pv(0, true, false, smem_i, phase);
        ...
        for (size_t iw=BN; iw<S; iw+=BN, ++smem_i) {
            ...
            gemm_pv(1, false, true, prev_smem_i, prev_phase);
            softmax(std::integral_constant<size_t, 0>{});
        ...
        gemm_pv(1, false, false, last_smem_i, last_phase);
...
template<int B, int H, int S, int D, int BM, int BN, int NUM_THREADS, int NUM_SMEM>
...
template<int B, int H, int S, int D, int BM, int BN, int NUM_THREADS, int NUM_SMEM>
void runAttnWSCXForN2StageKernel(fp16 *Q, fp16 *K, fp16 *V, fp16 *O) {
    CUtensorMap d_tma_map_Q = create_tensor_map<BM, D>(Q, B, H, S, D);
    CUtensorMap d_tma_map_K = create_tensor_map<BN, D>(K, B, H, S, D);
    CUtensorMap d_tma_map_V = create_tensor_map<BN, D>(V, B, H, S, D);
    CUtensorMap d_tma_map_O = create_tensor_map<BM, D>(O, B, H, S, D);
    ...
    auto* kernel = attnWSKCXForN2StageKernel<BM, BN, D, NUM_THREADS, NUM_SMEM>;
    constexpr size_t sMemSize = sizeof(SMemWS<BM, BN, D, NUM_SMEM>);
    static_assert(sMemSize < 256 * 1024);
    cudaCheck(cudaFuncSetAttribute(
        kernel,
...
// nvcc -std=c++17 -arch=sm_90a -O3 attn_dynamic_consumer.cu -o attn_test -lcuda -Xptxas=-v
// ncu --set full --launch-skip 100 --launch-count 1 ./attn_test
int main() {
    // constexpr int B = 1;
    // constexpr int H = 16;
    // constexpr int S = 114*128;
    // constexpr int D = 256;
    ...
    constexpr int BM = 128;
    constexpr int BN = 32;
    // auto *kernel = runAttnWSKernel<B, H, S, D, BM, BN, 256, 1>;
    // auto *kernel = runAttnWS2StageKernel<B, H, S, D, BM, BN, 256, 2>;
    auto *kernel = runAttnWSCXForNKernel<B, H, S, D, BM, BN, 256, 1>;
    // auto *kernel = runAttnWSCXForN2StageKernel<B, H, S, D, BM, BN, 256, 2>;
