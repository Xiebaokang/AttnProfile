...
void attnWSKCXForNKernel(
    ...
    // constexpr int Q_TILE_M = BM / N;
    // constexpr int N = 1;
    static_assert(NUM_SMEM >= 1);
    static_assert(N == 2);
    static_assert(BM % N == 0);
    static_assert(PRODUCER_REG_DEALLOC % 8 == 0);
    static_assert(CONSUMER_REG_ALLOC % 8 == 0);
    ...
    if (wg_idx == 2) {  // producer
        ...
        auto gemm_pv = [&](auto acc_o_i_const, auto wait_vfull_const, auto arrive_vempty_const, int smem_i, int phase) {
            constexpr size_t acc_o_i = decltype(acc_o_i_const)::value;
        ...
        // main loop
        for (size_t xxx=1; xxx<N*S/BN; xxx++) {
            if (xxx & 1) {
                gemm_qk(std::integral_constant<size_t, 1>{}, smem_i, phase);
                gemm_pv(std::integral_constant<size_t, 0>{}, std::true_type{}, std::false_type{}, smem_i, phase);
                softmax(std::integral_constant<size_t, 1>{});
            } else {
                int prev_smem_i, prev_phase;
                if constexpr (NUM_SMEM == 2) {
                    smem_i ^= 1;
                    if (smem_i == 0) { phase ^= 1; }
                    prev_smem_i = smem_i ^ 1;
                    prev_phase = phase ^ (smem_i == 0);
                } else {
                    smem_i++;
                    if (smem_i >= NUM_SMEM) { smem_i = 0; phase ^= 1; }
                    prev_smem_i = (smem_i + NUM_SMEM - 1) % NUM_SMEM;
                    prev_phase = phase;
                    if (prev_smem_i == NUM_SMEM - 1) { prev_phase ^= 1; }
                }
                gemm_qk(std::integral_constant<size_t, 0>{}, smem_i, phase);

                gemm_pv(std::integral_constant<size_t, 1>{}, std::false_type{}, std::true_type{}, prev_smem_i, prev_phase);
                softmax(std::integral_constant<size_t, 0>{});
            }
        }

        // Epilogue
        gemm_pv(std::integral_constant<size_t, 1>{}, std::false_type{}, std::true_type{}, smem_i, phase);
...
void attnWSKCXForNHalfQKernel(
    ...
    // setting shared memory
    extern __shared__ __align__(128) uint8_t smem[];
    SMemWSHalfQ<BM, BN, DIM, NUM_SMEM> &s = *reinterpret_cast<SMemWSHalfQ<BM, BN, DIM, NUM_SMEM>*>(smem);
    fp16 *sQ = s.Q, *sK = s.K, *sV = s.V, *sO = s.O;
    Barrier *Qempty = &s.Qempty, *Qfull = &s.Qfull;
    Barrier *Kempty = s.Kempty, *Vempty = s.Vempty, *Kfull = s.Kfull, *Vfull = s.Vfull;
    ...
    if (wg_idx == 2) {  // producer
        ...
        if (tid == 256) {
            ...
            for (size_t iw=0; iw<S; iw+=BN) {
                for (size_t n=0; n<N; ++n) {
                    const int q_phase = static_cast<int>(n & 1);
                    wait(Qempty, q_phase);
                    expect_bytes(Qfull, Q_TILE_M * DIM * sizeof(fp16));
                    load_async(sQ, &tensorMapQ, Qfull, bs, hn, by * BM + n * Q_TILE_M, 0);
                }
            }
        }
    } else {  // consumer
        warpgroup_reg_alloc<CONSUMER_REG_ALLOC>();
        // Bootstrap empty-smem_i barriers so producer can issue the first Q/K/V loads.
        arrive(Qempty);
        #pragma unroll
        for (int st = 0; st < NUM_SMEM; ++st) {
            arrive(&Kempty[st]);
        ...
        auto gemm_qk = [&](auto n_const, int smem_i, int phase) {
            constexpr size_t n = decltype(n_const)::value;
            constexpr int q_phase = static_cast<int>(n & 1);
            fp16 *KAddr = sK + smem_i * BN * DIM;
            #pragma unroll
            for (size_t j=0; j<BN/QK_MMA_N; j++) {
                #pragma unroll
            ...
            for (size_t j=0; j<BN/QK_MMA_N; j++) {
                #pragma unroll
                for (size_t k=0; k<DIM; k+=MMA_K) {
                    int q_row = wg_idx * MMA_M;
                    int q_col = k;
                    int k_row = j * QK_MMA_N;
                    int k_col = k;
                    fp16 *_QAddr = sQ + tma_smem_offset_2d<Q_TILE_M>(q_row, q_col);
                    fp16 *_KAddr = KAddr + tma_smem_offset_2d<BN>(k_row, k_col);
                    wgmma_ss<QK_MMA_N, 1, 1, 1, 0, 0, 16, 1024, true>(acc_s[j], _QAddr, _KAddr);
                }
            }
...
>
__global__  __launch_bounds__(NUM_THREADS)
void attnWSKCXForNHalfReuseQKernel(
    int B, int H, int S, 
    const __grid_constant__ CUtensorMap tensorMapQ, 
    const __grid_constant__ CUtensorMap tensorMapK, 
    const __grid_constant__ CUtensorMap tensorMapV, 
    ...
    if (wg_idx == 2) {  // producer
        ...
        auto gemm_qk = [&](auto n_const, auto wait_qfull_const, auto wait_kfull_const, auto release_qempty_const, auto release_kempty_const, int smem_i, int phase) {
            constexpr size_t n = decltype(n_const)::value;
            constexpr bool wait_qfull = decltype(wait_qfull_const)::value;
            constexpr bool wait_kfull = decltype(wait_kfull_const)::value;
            constexpr bool release_qempty = decltype(release_qempty_const)::value;
            constexpr bool release_kempty = decltype(release_kempty_const)::value;
            constexpr int q_phase = static_cast<int>(n & 1);
            fp16 *KAddr = sK + smem_i * BN * DIM;
            #pragma unroll
            for (size_t j=0; j<BN/QK_MMA_N; j++) {
            ...
            if constexpr (wait_qfull) { wait(Qfull, q_phase); }
            if constexpr (wait_kfull) { wait(&Kfull[smem_i], phase); }
            warpgroup_arrive();
            #pragma unroll
            for (size_t j=0; j<BN/QK_MMA_N; j++) {
                #pragma unroll
            ...
            warpgroup_commit_batch();
            warpgroup_wait();
            if constexpr (release_qempty) { arrive(Qempty); }
            if constexpr (release_kempty) { arrive(&Kempty[smem_i]); }
        };
            ...
            if ((xxx & 1) == 0) {
                ...
                if (xxx & 2) {
                    ...
                    gemm_qk(
                        ...
                        smem_i,
                        phase);
                    gemm_pv(std::integral_constant<size_t, 0>{}, std::false_type{}, std::true_type{}, prev_smem_i, prev_phase);
                    softmax(std::integral_constant<size_t, 0>{});
                }
            } else {
                if (xxx & 2) {
                    gemm_qk(
                        ...
                        smem_i,
                        phase);
                    gemm_pv(std::integral_constant<size_t, 1>{}, std::true_type{}, std::false_type{}, smem_i, phase);
                    softmax(std::integral_constant<size_t, 0>{});
                } else {
                    gemm_qk(
                        std::integral_constant<size_t, 1>{},
        ...
        if ((S / BN) & 1) {
            ...
            gemm_pv(std::integral_constant<size_t, 0>{}, std::false_type{}, std::true_type{}, smem_i, phase);
        }
...
template<
    int B, int H, int S, int D=128, int BM=128, int BN=128,
    int NUM_THREADS=384, int NUM_SMEM=2,
    uint32_t PRODUCER_REG_DEALLOC = 24,
    uint32_t CONSUMER_REG_ALLOC = 240
>
...
void runAttnWSCXForNKernel(fp16 *Q, fp16 *K, fp16 *V, fp16 *O) {
    ...
    auto* kernel = attnWSKCXForNKernel<
        BM, BN, D, NUM_THREADS, NUM_SMEM,
        PRODUCER_REG_DEALLOC, CONSUMER_REG_ALLOC>;
    constexpr size_t sMemSize = sizeof(SMemWS<BM, BN, D, NUM_SMEM>);
    static_assert(sMemSize < 256 * 1024);
...
template<
    ...
    uint32_t CONSUMER_REG_ALLOC = 240
>
void runAttnWSCXForNHalfQKernel(fp16 *Q, fp16 *K, fp16 *V, fp16 *O) {
    constexpr int Q_TILE_M = BM / 2;
    CUtensorMap d_tma_map_Q = create_tensor_map<Q_TILE_M, D>(Q, B, H, S, D);
    CUtensorMap d_tma_map_K = create_tensor_map<BN, D>(K, B, H, S, D);
    CUtensorMap d_tma_map_V = create_tensor_map<BN, D>(V, B, H, S, D);
    CUtensorMap d_tma_map_O = create_tensor_map<BM, D>(O, B, H, S, D);

    auto* kernel = attnWSKCXForNHalfQKernel<
        BM, BN, D, NUM_THREADS, NUM_SMEM,
        PRODUCER_REG_DEALLOC, CONSUMER_REG_ALLOC>;
    constexpr size_t sMemSize = sizeof(SMemWSHalfQ<BM, BN, D, NUM_SMEM>);
    static_assert(sMemSize < 256 * 1024);
    cudaCheck(cudaFuncSetAttribute(
        kernel,
        cudaFuncAttributeMaxDynamicSharedMemorySize, sMemSize));
...
template<
    ...
    uint32_t CONSUMER_REG_ALLOC = 240
>
void runAttnWSCXForNHalfReuseQKernel(fp16 *Q, fp16 *K, fp16 *V, fp16 *O) {
    constexpr int Q_TILE_M = BM / 2;
    CUtensorMap d_tma_map_Q = create_tensor_map<Q_TILE_M, D>(Q, B, H, S, D);
    CUtensorMap d_tma_map_K = create_tensor_map<BN, D>(K, B, H, S, D);
    CUtensorMap d_tma_map_V = create_tensor_map<BN, D>(V, B, H, S, D);
    ...
    auto* kernel = attnWSKCXForNHalfReuseQKernel<
        BM, BN, D, NUM_THREADS, NUM_SMEM,
        PRODUCER_REG_DEALLOC, CONSUMER_REG_ALLOC>;
    constexpr size_t sMemSize = sizeof(SMemWSHalfQ<BM, BN, D, NUM_SMEM>);
    static_assert(sMemSize < 256 * 1024);
...
template<
    int B, int H, int S, int D, int BM, int BN,
    bool USE_SEGMENTED,
    uint32_t PRODUCER_REG_DEALLOC,
    uint32_t CONSUMER_REG_ALLOC,
    int NUM_THREADS=384, int NUM_SMEM=2
>
void benchmarkAttnRegConfig() {
    printf("\n[reg-tune] kernel=%s dealloc=%u alloc=%u\n",
           USE_SEGMENTED ? "attnWSKCXForNKernel" : "attnWS2StageKernel",
           PRODUCER_REG_DEALLOC,
           CONSUMER_REG_ALLOC);

    if constexpr (USE_SEGMENTED) {
        auto *kernel = runAttnWSCXForNHalfQKernel<
            B, H, S, D, BM, BN, NUM_THREADS, NUM_SMEM,
            PRODUCER_REG_DEALLOC, CONSUMER_REG_ALLOC>;
        benchmark_attn<B, H, S, D, BM, BN>(kernel);
    } else {
        auto *kernel = runAttnWS2StageKernel<
            B, H, S, D, BM, BN, NUM_THREADS, NUM_SMEM,
            PRODUCER_REG_DEALLOC, CONSUMER_REG_ALLOC>;
        benchmark_attn<B, H, S, D, BM, BN>(kernel);
    }
