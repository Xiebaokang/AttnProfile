...
void attnWSKCXForNSegmentedKernel(
    ...
    if (wg_idx == 2) {  // producer
        ...
        auto gemm_qk = [&](size_t n, int smem_i, int phase) {
        ...
        auto gemm_pv = [&](auto acc_o_i_const, bool wait_vfull, bool arrive_vempty, int smem_i, int phase) {
        ...
        // Prologue
        gemm_qk(0, smem_i, phase);
        softmax(std::integral_constant<size_t, 0>{});
        // gemm_qk(1, smem_i, phase);
        ...
        for (size_t xxx=1; xxx<N*S/BN; xxx++) {
            // size_t iw = xxx / N * BN;
            const size_t tile = xxx / N;
            const size_t n = xxx % N;
            smem_i = tile % NUM_SMEM;
            phase = (tile / NUM_SMEM) & 1;

            gemm_qk(n, smem_i, phase);

            const size_t prev_xxx = xxx - 1;
            const size_t prev_tile = prev_xxx / N;
            const size_t prev_n = prev_xxx % N;
            const int prev_smem_i = prev_tile % NUM_SMEM;
            const int prev_phase = (prev_tile / NUM_SMEM) & 1;
            if (prev_n == 0) {
                gemm_pv(std::integral_constant<size_t, 0>{}, true, N == 1, prev_smem_i, prev_phase);
            } else {
                gemm_pv(std::integral_constant<size_t, 1>{}, false, N == 2, prev_smem_i, prev_phase);
            }
            if (n == 0) {
                softmax(std::integral_constant<size_t, 0>{});
            } else {
        ...
        // Epilogue
        // smem_i++;
        const size_t last_xxx = N*S/BN - 1;
        const size_t last_tile = last_xxx / N;
        // const size_t last_n = last_xxx % N;
        const int last_smem_i = last_tile % NUM_SMEM;
        const int last_phase = (last_tile / NUM_SMEM) & 1;
        // if (last_n == 0) {
            // gemm_pv(std::integral_constant<size_t, 0>{}, true, N == 1, last_smem_i, last_phase);
        // } else {
            gemm_pv(std::integral_constant<size_t, 1>{}, false, N == 2, last_smem_i, last_phase);
        // }
...
// nvcc -std=c++17 -arch=sm_90a -O3 attn_111.cu -o attn_test -lcuda -Xptxas=-v
