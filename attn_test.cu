...
void attnWSKCXForNSegmentedKernel(
    ...
    if (wg_idx == 2) {  // producer
        ...
        for (size_t iw=BN; iw<S; iw+=BN, ++smem_i) {
            ...
            gemm_pv(0, true, true, prev_smem_i, prev_phase);            
            softmax(std::integral_constant<size_t, 0>{});
        ...
        gemm_pv(0, true, true, last_smem_i, last_phase);
    ...
    if (wg_idx == 2) {  // producer
        ...
        if (tid == 256) {
            for (size_t rep=0; rep<2; rep++) {
                ...
                // load Q
                expect_bytes(Qmbar, BM * DIM * sizeof(fp16));
                load_async(sQ, &tensorMapQ, Qmbar, bs, hn, (by * 2 + rep) * BM, 0);
                for (size_t iw=0; iw<S; iw+=BN, smem_i++) {
                    if (smem_i == NUM_SMEM) { smem_i = 0; phase ^= 1; }
                    fp16 *addrK = sK + smem_i * BN * DIM;
                    fp16 *addrV = sV + smem_i * BN * DIM;
            ...
            for (size_t rep=0; rep<2; rep++) {
                ...
                // tma store
                if (tid == 0) {
                    store_async(&tensorMapO, sO, bs, hn, (by * 2 + rep) * BM, 0);
                    tma_store_arrive();
                }
            }    
    }
...
void runAttnWSCXKernel(fp16 *Q, fp16 *K, fp16 *V, fp16 *O) {
    ...
    auto* kernel = attnWSKCXKernel<BM, BN, D, NUM_THREADS, NUM_SMEM>;
    constexpr size_t sMemSize = sizeof(SMemWS<BM, BN, D, NUM_SMEM>);
    static_assert(sMemSize < 256 * 1024);
    cudaCheck(cudaFuncSetAttribute(
        kernel,
...
// nvcc -std=c++17 -arch=sm_90a -O3 attn_test.cu -o attn -lcuda -Xptxas=-v
// ncu --set full --launch-skip 100 --launch-count 1 ./attn
int main() {
    // constexpr int B = 1;
    // constexpr int H = 16;
    // constexpr int S = 114*256;
    // // constexpr int S = 114*512;
    // constexpr int D = 64;
    ...
    auto *kernel = runAttnWSCXKernel<B, H, S, D, BM, BN, 384, 2>;
