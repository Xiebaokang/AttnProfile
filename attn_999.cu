...
void runAttnWSCXForNKernel(fp16 *Q, fp16 *K, fp16 *V, fp16 *O) {
    ...
    dim3 grid = {static_cast<unsigned int>(S/BM/2), static_cast<unsigned int>(H), static_cast<unsigned int>(B)};
...
template<
    ...
    uint32_t CONSUMER_REG_ALLOC = 240
>
void runAttnWSCXForNKernel_(fp16 *Q, fp16 *K, fp16 *V, fp16 *O) {
    CUtensorMap d_tma_map_Q = create_tensor_map<BM, D>(Q, B, H, S, D);
    CUtensorMap d_tma_map_K = create_tensor_map<BN, D>(K, B, H, S, D);
    CUtensorMap d_tma_map_V = create_tensor_map<BN, D>(V, B, H, S, D);
    CUtensorMap d_tma_map_O = create_tensor_map<BM, D>(O, B, H, S, D);

    auto* kernel = attnWSKCXForNKernel_<
        BM, BN, D, NUM_THREADS, NUM_SMEM,
        PRODUCER_REG_DEALLOC, CONSUMER_REG_ALLOC>;
    constexpr size_t sMemSize = sizeof(SMemWS<BM, BN, D, NUM_SMEM>);
    static_assert(sMemSize < 256 * 1024);
...
void benchmarkAttnRegSweep() {
    // benchmarkAttnRegConfig<B, H, S, D, BM, BN, 24, 184, NUM_THREADS, NUM_SMEM>();
    // benchmarkAttnRegConfig<B, H, S, D, BM, BN, 32, 184, NUM_THREADS, NUM_SMEM>();
    // benchmarkAttnRegConfig<B, H, S, D, BM, BN, 24, 192, NUM_THREADS, NUM_SMEM>();
    // benchmarkAttnRegConfig<B, H, S, D, BM, BN, 32, 192, NUM_THREADS, NUM_SMEM>();
    // benchmarkAttnRegConfig<B, H, S, D, BM, BN, 24, 208, NUM_THREADS, NUM_SMEM>();
    // benchmarkAttnRegConfig<B, H, S, D, BM, BN, 32, 208, NUM_THREADS, NUM_SMEM>();
    // benchmarkAttnRegConfig<B, H, S, D, BM, BN, 24, 216, NUM_THREADS, NUM_SMEM>();
    // benchmarkAttnRegConfig<B, H, S, D, BM, BN, 32, 216, NUM_THREADS, NUM_SMEM>();
    benchmarkAttnRegConfig<B, H, S, D, BM, BN, 24, 224, NUM_THREADS, NUM_SMEM>();
    benchmarkAttnRegConfig<B, H, S, D, BM, BN, 32, 224, NUM_THREADS, NUM_SMEM>();
    benchmarkAttnRegConfig<B, H, S, D, BM, BN, 24, 240, NUM_THREADS, NUM_SMEM>();
}

// export CUDA_VISIBLE_DEVICES=1
// nvcc -std=c++17 -arch=sm_90a -O3 attn_999.cu -o attn_999 -lcuda -Xptxas=-v
// ncu --set full --launch-skip 100 --launch-count 1 ./attn_999
int main() {
    constexpr int BM = 256;
    constexpr int BN = 112;
    constexpr int B = 1;
    constexpr int H = 16;
    constexpr int S = 240*112;
    constexpr int D = 64;
    ...
    auto *kernel = runAttnWSCXForNKernel_<B, H, S, D, BM, BN, 384, 2, 24, 240>;
    // verify_attn<B, H, S, D, BN>(kernel);
    benchmark_attn<B, H, S, D, BM, BN>(kernel);
    // benchmarkAttnRegSweep<B, H, S, D, BM, BN>();
