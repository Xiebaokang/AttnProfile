...
void attnWSKCXForNSegmentedKernel(
    ...
    static_assert(N == 1, "attn_loop_2stage_pipeline_bm128.cu is specialized for BM=128.");
    ...
    if (wg_idx == 2) {  // producer
        ...
        auto gemm_qk = [&](int smem_i, int phase) {
        ...
        auto gemm_pv = [&](int smem_i, int phase) {
        ...
        // Prologue
        gemm_qk(smem_i, phase);
        softmax();

        // Main loop: compute current QK while consuming previous V tile, then
        // update the current tile softmax state for the next iteration.
        smem_i++;
        for (size_t iw=BN; iw<S; iw+=BN, ++smem_i) {
            ...
            gemm_pv(prev_smem_i, prev_phase);
            softmax();
        }
        ...
        gemm_pv(last_smem_i, last_phase);
...
// nvcc -std=c++17 -arch=sm_90a -O3 attn_loop_2stage_pipeline_bm128.cu -o attn_test -lcuda -Xptxas=-v
