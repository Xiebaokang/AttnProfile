...
void runAttnWS2StageKernel(fp16 *Q, fp16 *K, fp16 *V, fp16 *O) {
    ...
    auto* kernel = attnWS2StageKernel<
        ...
        PRODUCER_REG_DEALLOC, CONSUMER_REG_ALLOC, P_SMEM_K_TILES>;
    constexpr size_t sMemSize = sizeof(SMemWS<BM, BN, D, NUM_SMEM, P_SMEM_K_TILES>);
    // printf("[D] smem size: %ld\n", sMemSize);
...
void benchmarkAttnRegConfig() {
    ...
    if constexpr (USE_SEGMENTED) {
        auto *kernel = runAttnWSCXForNKernel<
...
void benchmarkAttnRegSweep() {
    // benchmarkAttnRegConfig<B, H, S, D, BM, BN, USE_SEGMENTED, 24, 184, NUM_THREADS, NUM_SMEM, P_SMEM_K_TILES>();
    // benchmarkAttnRegConfig<B, H, S, D, BM, BN, USE_SEGMENTED, 32, 184, NUM_THREADS, NUM_SMEM, P_SMEM_K_TILES>();
    // benchmarkAttnRegConfig<B, H, S, D, BM, BN, USE_SEGMENTED, 24, 192, NUM_THREADS, NUM_SMEM, P_SMEM_K_TILES>();
    // benchmarkAttnRegConfig<B, H, S, D, BM, BN, USE_SEGMENTED, 32, 192, NUM_THREADS, NUM_SMEM, P_SMEM_K_TILES>();
    // benchmarkAttnRegConfig<B, H, S, D, BM, BN, USE_SEGMENTED, 24, 208, NUM_THREADS, NUM_SMEM, P_SMEM_K_TILES>();
    // benchmarkAttnRegConfig<B, H, S, D, BM, BN, USE_SEGMENTED, 32, 208, NUM_THREADS, NUM_SMEM, P_SMEM_K_TILES>();
    // benchmarkAttnRegConfig<B, H, S, D, BM, BN, USE_SEGMENTED, 24, 216, NUM_THREADS, NUM_SMEM, P_SMEM_K_TILES>();
    // benchmarkAttnRegConfig<B, H, S, D, BM, BN, USE_SEGMENTED, 32, 216, NUM_THREADS, NUM_SMEM, P_SMEM_K_TILES>();
    // benchmarkAttnRegConfig<B, H, S, D, BM, BN, USE_SEGMENTED, 24, 224, NUM_THREADS, NUM_SMEM, P_SMEM_K_TILES>();
    benchmarkAttnRegConfig<B, H, S, D, BM, BN, USE_SEGMENTED, 32, 224, NUM_THREADS, NUM_SMEM, P_SMEM_K_TILES>();
    benchmarkAttnRegConfig<B, H, S, D, BM, BN, USE_SEGMENTED, 24, 240, NUM_THREADS, NUM_SMEM, P_SMEM_K_TILES>();
}


// export CUDA_VISIBLE_DEVICES=1
// nvcc -std=c++17 -arch=sm_90a -O3 attn_clock.cu -o attn_clock -lcuda -Xptxas=-v
// ncu --set full --launch-skip 100 --launch-count 1 ./attn_clock
int main() {
    ...
    auto *kernel = runAttnWS2StageKernel<B, H, S, D, BM, BN, 384, 1, 24, 240, 11>;
    // auto *kernel = runAttnWSCXForNKernel<B, H, S, D, BM, BN, 384, 2, 24, 240, 4>;
