...
void benchmarkAttnRegSweep() {
    ...
    // benchmarkAttnRegConfig<B, H, S, D, BM, BN, USE_SEGMENTED, 40, 208, NUM_THREADS, NUM_SMEM>();
    benchmarkAttnRegConfig<B, H, S, D, BM, BN, USE_SEGMENTED, 48, 208, NUM_THREADS, NUM_SMEM>();
    ...
    // benchmarkAttnRegConfig<B, H, S, D, BM, BN, USE_SEGMENTED, 24, 256, NUM_THREADS, NUM_SMEM>();
    // benchmarkAttnRegConfig<B, H, S, D, BM, BN, USE_SEGMENTED, 32, 224, NUM_THREADS, NUM_SMEM>();
    // benchmarkAttnRegConfig<B, H, S, D, BM, BN, USE_SEGMENTED, 32, 240, NUM_THREADS, NUM_SMEM>();
    // benchmarkAttnRegConfig<B, H, S, D, BM, BN, USE_SEGMENTED, 40, 224, NUM_THREADS, NUM_SMEM>();
    // benchmarkAttnRegConfig<B, H, S, D, BM, BN, USE_SEGMENTED, 40, 240, NUM_THREADS, NUM_SMEM>();
}

// export CUDA_VISIBLE_DEVICES=1
// nvcc -std=c++17 -arch=sm_90a -O3 attn_777.cu -o attn -lcuda -Xptxas=-v
// ncu --set full --launch-skip 100 --launch-count 1 ./attn
int main() {
    constexpr int B = 1;
    constexpr int H = 16;
    ...
    // auto *kernel = runAttnWS2StageKernel<B, H, S, D, BM, BN, 384, 3, 24, 240>;
    auto *kernel = runAttnWSCXForNKernel<B, H, S, D, BM, BN, 384, 3, 32, 208>;
