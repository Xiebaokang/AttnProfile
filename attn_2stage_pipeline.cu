...
void attnWS2StageKernel(
    ...
    if (wg_idx == 2) {  // producer
        warpgroup_reg_dealloc<48>();
        if (tid == 256) {
            int smem_i = 0, phase = 0, next_phase = 0;
            // load Q
            ...
            for (size_t iw=0; iw<S; iw+=BN, ++smem_i) {
                ...
                int next_smem_i = (smem_i + NUM_SMEM + 1) % NUM_SMEM;
                // int next_phase = phase;
                if (smem_i + 1 == NUM_SMEM) { next_phase ^= 1; }
                fp16 *VAddr = sV + next_smem_i * BN * DIM;

                // load V
                wait(&Vempty[next_smem_i], next_phase);
                expect_bytes(&Vfull[next_smem_i], BN * DIM * sizeof(fp16));
                load_async(VAddr, &tensorMapV, &Vfull[next_smem_i], bs, hn, iw, 0);
            }
        }
    } else {  // consumer
        warpgroup_reg_alloc<216>();
        ...
        // ==== epilogue ====
        int last_v_tile = S / BN - 1;
        int last_smem_i = (last_v_tile + 1) % NUM_SMEM;
        int last_phase = ((last_v_tile + 1) / NUM_SMEM) & 1;
...
void attnWS2StageKernel__(
    ...
    if (wg_idx == 2) {  // producer
        ...
        // ==== main loop ====
        int prev_v_smem_i = smem_i;
        int prev_v_phase = phase;
        for (size_t iw=BN; iw<S; iw+=BN) {
            ...
            // prev gemm-pv
            __gemm_pv(prev_v_smem_i, prev_v_phase);

            // softmax
            __softmax();

            prev_v_smem_i = smem_i;
            prev_v_phase = phase;
        }
...
template<int B, int H, int S, int D=128, int BM=128, int BN=128, int NUM_THREADS=384, int NUM_SMEM=2>
void runAttnWS2StageKernel_(fp16 *Q, fp16 *K, fp16 *V, fp16 *O) {
    CUtensorMap d_tma_map_Q = create_tensor_map<BM, D>(Q, B, H, S, D);
    CUtensorMap d_tma_map_K = create_tensor_map<BN, D>(K, B, H, S, D);
    CUtensorMap d_tma_map_V = create_tensor_map<BN, D>(V, B, H, S, D);
    CUtensorMap d_tma_map_O = create_tensor_map<BM, D>(O, B, H, S, D);
    ...
    auto* kernel = attnWS2StageKernel_<BM, BN, D, NUM_THREADS, NUM_SMEM>;
    constexpr size_t sMemSize = sizeof(SMemWS<BM, BN, D, NUM_SMEM>);
    static_assert(sMemSize < 256 * 1024);
    cudaCheck(cudaFuncSetAttribute(
        kernel,
...
template<int B, int H, int S, int D=128, int BM=128, int BN=128, int NUM_THREADS=384, int NUM_SMEM=2>
void runAttnWS2StageKernel__(fp16 *Q, fp16 *K, fp16 *V, fp16 *O) {
    CUtensorMap d_tma_map_Q = create_tensor_map<BM, D>(Q, B, H, S, D);
    CUtensorMap d_tma_map_K = create_tensor_map<BN, D>(K, B, H, S, D);
    CUtensorMap d_tma_map_V = create_tensor_map<BN, D>(V, B, H, S, D);
    CUtensorMap d_tma_map_O = create_tensor_map<BM, D>(O, B, H, S, D);
    ...
    auto* kernel = attnWS2StageKernel__<BM, BN, D, NUM_THREADS, NUM_SMEM>;
    constexpr size_t sMemSize = sizeof(SMemWS<BM, BN, D, NUM_SMEM>);
    static_assert(sMemSize < 256 * 1024);
    cudaCheck(cudaFuncSetAttribute(
        kernel,
...
// nvcc -std=c++17 -arch=sm_90a -O3 attn_2stage_pipeline.cu -o attn -lcuda -Xptxas=-v
...
int main() {
    ...
    // auto *kernel = runAttnWS2StageKernel<B, H, S, D, BM, BN, 384>;
    auto *kernel = runAttnWS2StageKernel_<B, H, S, D, BM, BN, 384>;
    // auto *kernel = runAttnWS2StageKernel__<B, H, S, D, BM, BN, 384>;
