...
void attnWSKForNKernel(
    ...
    if (wg_idx == 2) {  // producer
        ...
        if (tid == 256) {
            ...
            if constexpr (Q_SMEM_K_TILES > 0) {
                ...
                for (int q_row = 0; q_row < BM; q_row += QO_TMA_M) {
                    #pragma unroll
                    for (int q_col = 0; q_col < Q_SMEM_COLS; q_col += QO_TMA_N) {
        ...
        uint32_t q_regs[N][Q_REG_STORAGE_TILES][4];
        ...
        if constexpr (Q_REG_TILES > 0) {
            ...
            for (size_t n=0; n<N; n++) {
                ...
                for (size_t qt=0; qt<Q_REG_TILES; qt++) {
                    const int q_row = by * BM + wg_idx * N * MMA_M + n * MMA_M;
                    const int q_col = Q_SMEM_COLS + qt * MMA_K;
                    load_q_global_fragment<DIM>(q_regs[n][qt], Q, H, SQO, bs, hn, q_row, q_col);
                }
            }
        }
                ...
                for (size_t k=0; k<DIM; k+=MMA_K) {
                    ...
                    if constexpr (Q_REG_TILES == 0) {
                        ...
                        wgmma_rs<QK_MMA_N, 1, 1, 1, 0, 0, 16, 1024, true>(
                            acc_s[j], q_regs[n][q_reg_tile], _KAddr);
...
template<
    ...
    int NUM_THREADS=384
>
void runAttnWSForNKernel(fp16 *Q, fp16 *K, fp16 *V, fp16 *O) {
    constexpr int QO_TMA_M = 128;
    constexpr int QO_TMA_N = 64;
    static_assert(BM % QO_TMA_M == 0);
    ...
    auto* kernel = attnWSKForNKernel<BM, BN, D, NUM_THREADS, NUM_SMEM, PRODUCER_REG_DEALLOC, CONSUMER_REG_ALLOC, P_SMEM_K_TILES, Q_REG_K_TILES>;
