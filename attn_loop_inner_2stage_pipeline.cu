    ...
    if (wg_idx == 2) {  // producer
        ...
        for (size_t iw=0; iw<S; iw+=BN, phase ^= 1) {
            ...
            for (size_t n=0; n<N; n++) {
                ...
                auto gemm_pv = [&](size_t acc_o_i, bool wait_vfull, bool arrive_vempty) {
                ...
                if ((iw >= BN) || (n == 1 && iw == 0)) {
                    gemm_pv((n + N - 1) % N, n == N-1, n == 0);
                }

                softmax();
            
                if (iw == S-BM && n == 1) {
                    gemm_pv(1, false, false);
                }
            }
        }
        // acc_o = acc_o / logsum
...
void runAttnWSCXForNKernel(fp16 *Q, fp16 *K, fp16 *V, fp16 *O) {
    ...
    auto* kernel = attnWSKCXForNKernel<BM, BN, D, NUM_THREADS>;
...
// nvcc -std=c++17 -arch=sm_90a -O3 attn_loop_inner_2stage_pipeline.cu -o attn_test -lcuda -Xptxas=-v
