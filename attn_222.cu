...
template<int BM, int BN, int DIM, int NUM_THREADS, int NUM_SMEM, bool FUSE_SEGMENT_LOOP=false>
...
void attnWSKCXForNSegmentedKernel(
    ...
    if (wg_idx == 2) {  // producer
        ...
        auto gemm_pv = [&](size_t acc_o_i, auto wait_vfull_const, auto arrive_vempty_const, int smem_i, int phase) {
        ...
        if constexpr (FUSE_SEGMENT_LOOP) {
            static_assert(N == 2);
            ...
            // main loop
            for (size_t iter=1; iter<N*S/BN; ++iter) {
                if (iter & 1) {
                    gemm_qk(std::integral_constant<size_t, 1>{}, smem_i, phase);
                    gemm_pv(0, std::true_type{}, std::false_type{}, smem_i, phase);
                    ...
                    gemm_pv(1, std::false_type{}, std::true_type{}, prev_smem_i, prev_phase);
                    softmax(std::integral_constant<size_t, 0>{});
                }
            }

            // Epilogue
            gemm_pv(1, std::false_type{}, std::true_type{}, smem_i, phase);
        } else {
            // Prologue
            gemm_qk(std::integral_constant<size_t, 0>{}, smem_i, phase);
            softmax(std::integral_constant<size_t, 0>{});
            ...
            gemm_pv(1, std::false_type{}, std::true_type{}, last_smem_i, last_phase);
        }
...
__global__  __launch_bounds__(NUM_THREADS, 1)
void attnWSKCXForNSegmentedKernel_(
    int B, int H, int S, 
    const __grid_constant__ CUtensorMap tensorMapQ, 
    const __grid_constant__ CUtensorMap tensorMapK, 
    const __grid_constant__ CUtensorMap tensorMapV, 
    ...
    if (wg_idx == 2) {  // producer
        ...
        const int last_smem_i = (smem_i % NUM_SMEM + NUM_SMEM - 1) % NUM_SMEM;
        const int last_phase = phase;
        gemm_pv(1, false, true, last_smem_i, last_phase);
...
void runAttnWSCXForNKernel_(fp16 *Q, fp16 *K, fp16 *V, fp16 *O) {
    ...
    auto* kernel = attnWSKCXForNSegmentedKernel_<BM, BN, D, NUM_THREADS, NUM_SMEM>;
    constexpr size_t sMemSize = sizeof(SMemWS<BM, BN, D, NUM_SMEM>);
    static_assert(sMemSize < 256 * 1024);
    cudaCheck(cudaFuncSetAttribute(
        kernel,
...
template<int B, int H, int S, int D=128, int BM=128, int BN=128, int NUM_THREADS=384, int NUM_SMEM=2>
void runAttnWSCXForNKernelFused(fp16 *Q, fp16 *K, fp16 *V, fp16 *O) {
    CUtensorMap d_tma_map_Q = create_tensor_map<BM, D>(Q, B, H, S, D);
    CUtensorMap d_tma_map_K = create_tensor_map<BN, D>(K, B, H, S, D);
    CUtensorMap d_tma_map_V = create_tensor_map<BN, D>(V, B, H, S, D);
    CUtensorMap d_tma_map_O = create_tensor_map<BM, D>(O, B, H, S, D);

    auto* kernel = attnWSKCXForNSegmentedKernel<BM, BN, D, NUM_THREADS, NUM_SMEM, true>;
    constexpr size_t sMemSize = sizeof(SMemWS<BM, BN, D, NUM_SMEM>);
    static_assert(sMemSize < 256 * 1024);
    cudaCheck(cudaFuncSetAttribute(
        kernel,
...
// nvcc -std=c++17 -arch=sm_90a -O3 attn_222.cu -o attn -lcuda -Xptxas=-v
...
int main() {
    ...
    auto *kernel = runAttnWSCXForNKernel<B, H, S, D, BM, BN, 384, 3>;
    // auto *kernel = runAttnWSCXForNKernelFused<B, H, S, D, BM, BN, 384, 3>;
    // auto *kernel = runAttnWSCXForNKernel_<B, H, S, D, BM, BN, 384, 2>;
